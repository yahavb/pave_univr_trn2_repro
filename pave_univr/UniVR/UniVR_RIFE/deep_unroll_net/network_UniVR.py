import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__), 'RIFE'))
from IFNet_m import *
...
def generate_2D_grid(H, W, device=None):
    if device is None:
...
def interflow_syn(bidir_optical_flow, t, gamma=1.0, device=None):# -gamma/2 <= tau <= 1 - gamma/2
    _, _, H, W = bidir_optical_flow.size()

    grid_rows = generate_2D_grid(H, W, device=device)[1]
...
class UniVR(nn.Module):
    ...
    def forward(self, img, timestep=0.0, gamma=1.0, scale=1, scale_list=[4, 2, 1], TTA=False):
        ...
        _, _, H, W = img.size()
        grid_rows = generate_2D_grid(H, W, device=self.device)[1]
        grid_rows = grid_rows.unsqueeze(0).unsqueeze(0)
